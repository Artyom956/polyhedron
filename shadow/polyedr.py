...
class Edge:
    ...
    # Определяет, является ли ребро видимым
    def is_visible(self):
        if len(self.gaps) == 1 and self.gaps[0].beg == Edge.SBEG and self.gaps[0].fin == Edge.SFIN:
            return 1  # Ребро является видимым
        return 0  # Ребро является невидимым или частично видимым
    def is_parallel(self, delta = 0.000001):
        direction = self.fin - self.beg  # Вектор направления ребра
        if abs(direction.y) < delta and abs(direction.z) < delta or \
        abs(direction.x) < delta and abs(direction.z) < delta or \
        abs(direction.x) < delta and abs(direction.y) < delta:
            return 1
        return 0
class Facet:
    """ Грань полиэдра """
    ...
    def __init__(self, vertexes):
        ...
        self.edges = [Edge(vertexes[i-1], vertexes[i]) 
    ...
    # Проверка расположения центра
    def center_not_in_cube(self):
        c = self.center()
        return (abs(c.x) > 0.5 or abs(c.y) > 0.5 or abs(c.z) > 0.5)
    
    # Нахождение площади проекции грани
    def projection_area(self):
        projected = [R3(v.x, v.y, 0) for v in self.vertexes]
        area = 0.0
        n = len(projected)
        for i in range(n):
            j = (i + 1) % n
    ...
    def draw(self, tk):  # pragma: no cover
        ...
        for e in self.edges:
            for f in self.facets:
                e.shadow(f)    
        for e in self.edges:
            e.visibility = e.is_visible()
        for f in self.facets:        
            for e in f.edges:
                for g in self.facets:
                    e.shadow(g)  
                if e.is_visible() == 0 or e.is_parallel() == 0:
                    f.visibility = 0
                    break    
            if f.visibility == 1:
                if f.center_not_in_cube():
                    area = f.projection_area()
                    self.total_area += area    
        for e in self.edges:               
            for s in e.gaps:
                tk.draw_line(e.r3(s.beg), e.r3(s.fin))
        print(f"Площадь проекций искомых граней равна '{self.total_area}' ")        
